...
const State = {
  ...
  tts: { synth: window.speechSynthesis, voices: [], queue: [], idx: 0, speaking: false, paused: false, utter: null, rate: 1, keepAliveTimer: null, watchdogTimer: null, useBrave:true, braveAudio:null },
  hideTimer: null,
};
...
const el = {
  ...
  ttsBar: $('#ttsBar'), ttsStatus: $('#ttsStatus'), ttsKeepAlive: $('#ttsKeepAlive'), braveAudio: $('#braveAudio'), ttsMode: $('#ttsMode'),
};
...
function startBackgroundKeepAlive(){
  // Brave mode: audio element itself keeps background alive, no silent loop needed but keep for fallback
  if(State.tts.useBrave && el.braveAudio){ /* brave audio is the session */ }
  else if(el.ttsKeepAlive){ el.ttsKeepAlive.play().catch(()=>{}); }
  clearInterval(State.tts.keepAliveTimer);
  State.tts.keepAliveTimer = setInterval(()=>{
    if(!State.tts.speaking || State.tts.paused) return;
    if(!State.tts.useBrave && State.tts.synth){
...
function startTts(){
  ...
  if(el.ttsMode) el.ttsMode.textContent = State.tts.useBrave ? 'BRAVE BG ON' : 'NATIVE';
...
async function speakNext(){
  ...
  if(State.tts.useBrave && typeof BraveTTS !== 'undefined' && el.braveAudio){
    try {
      await BraveTTS.speak(spoken, State.tts.rate, el.braveAudio);
      if(State.tts.speaking && !State.tts.paused){ State.tts.idx++; speakNext(); }
      return;
    } catch(e){
      console.warn('BraveTTS fail, fallback to native', e);
      // fallback to native for this sentence
    }
  }
  // native fallback
...
function pauseTTS(){
  ...
  if(State.tts.useBrave && el.braveAudio){ BraveTTS.pause(el.braveAudio); }
  if(State.tts.synth) try{ State.tts.synth.pause(); }catch{}
  setTtsPlayIcon('play'); el.ttsStatus.textContent='일시정지'; stopBackgroundKeepAlive();
}
function resumeTTS(){
  if(!State.tts.speaking || !State.tts.paused) return;
  State.tts.paused = false;
  if(State.tts.useBrave && el.braveAudio){ BraveTTS.resume(el.braveAudio); }
  else if(State.tts.synth) try{ State.tts.synth.resume(); }catch{}
  setTtsPlayIcon('pause'); el.ttsStatus.textContent='읽는 중…'; startBackgroundKeepAlive();
  // if audio ended while paused, continue
  if(State.tts.useBrave && el.braveAudio && el.braveAudio.ended){ State.tts.idx++; speakNext(); }
}
function stopTTS(){
  State.tts.speaking = false; State.tts.paused = false;
  if(State.tts.synth) try{ State.tts.synth.cancel(); }catch{}
  if(el.braveAudio) BraveTTS.stop(el.braveAudio);
  if(lastTtsNode){ lastTtsNode.classList.remove('tts-active'); lastTtsNode = null; }
  el.ttsBar.classList.remove('show'); setTimeout(()=>el.ttsBar.hidden=true, 200);
  setTtsPlayIcon('play'); stopBackgroundKeepAlive();
}
function toggleTTS(){ if(!State.tts.speaking) startTts(); else if(State.tts.paused) resumeTTS(); else pauseTTS(); }
function setTtsPlayIcon(mode){ const b = $('#ttsPlay'); if(!b) return; b.innerHTML = mode==='play' ? '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>' : '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 4h4v16H6zM14 4h4v16h-4z"/></svg>'; }
function ttsJump(dir){
  if(!State.tts.speaking) return;
  if(State.tts.useBrave && el.braveAudio){ BraveTTS.stop(el.braveAudio); }
  if(State.tts.synth) try{ State.tts.synth.cancel(); }catch{}
  State.tts.idx = Math.max(0, Math.min(State.tts.queue.length-1, State.tts.idx + dir));
  speakNext();
}
window.ttsJump = ttsJump;
...
// --- FORCE NATIVE FIX for Brave NotSupportedError ---
(() => {
  const origBraveSpeak = (typeof BraveTTS!=='undefined' && BraveTTS.speak) ? BraveTTS.speak.bind(BraveTTS) : null;
  let audioCtx = null;
  function getCtx(){
  ...
  // Override BraveTTS.speak to use AudioContext decoding (bypasses <audio> NotSupportedError)
  if(typeof BraveTTS!=='undefined'){
    BraveTTS.speak = async function(text, rate, audioEl){
      const q = text.slice(0,180);
      const url = `https://translate.googleapis.com/translate_tts?ie=UTF-8&tl=ko&client=tw-ob&q=${encodeURIComponent(q)}`;
      try{
        ...
        // keep for pause/resume
        BraveTTS._src = src;
        src.start();
        return new Promise(res=>{ src.onended=()=>res(); });
      }catch(e){
        console.warn('AudioContext MP3 fail, fallback to native', e);
        // fallback to native speechSynthesis (which log shows works)
    ...
    BraveTTS.stop = function(){ try{ BraveTTS._src&&BraveTTS._src.stop(); }catch{}; try{ speechSynthesis.cancel(); }catch{} };
  }
  // Force settings to use Brave mode but our fixed version will actually use AudioContext/native
  if(typeof State!=='undefined' && State.tts) State.tts.useBrave = true;
})();
