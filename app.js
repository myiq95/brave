...
// ==================== TTS OVERRIDE - TEST-READER LOGIC (VERIFIED WORKING) ====================
(() => {
  console.log('[TTS OVERRIDE] test-reader logic loaded');
  
  // Override chunk and speak logic
  ...
  function clearHighlight(){
    document.querySelectorAll('.tts-highlight, .highlight').forEach(e=>{
  ...
  function highlightSentence(n){
    clearHighlight();
    if(n<0 || !State.tts.queue[n]) return;
    ...
    while(node = walker.nextNode()){
      if(node.textContent.includes(snippet.slice(0,15))){
        const parent = node.parentElement;
        if(parent){
          parent.classList.add('tts-highlight');
          parent.scrollIntoView({behavior:'smooth', block:'center'});
          // also add inline style for visibility
          parent.style.background='#ffeb3b';
          break;
        }
      }
    }
  ...
  function speakOnce(text){
    return new Promise(res=>{
      ...
      const voiceName = State.settings.ttsVoice;
      if(voiceName){
  ...
  window.startTts = async function(){
    try{
      // unlock audio
      try{
      ...
      const flow = document.getElementById('bookFlow');
      if(!flow){ alert('책을 먼저 열어주세요'); return; }
      const text = (flow.innerText||flow.textContent||'').trim();
      if(!text){ alert('읽을 내용이 없습니다'); return; }
      State.tts.queue = simpleChunk(text);
      State.tts.idx = 0;
      State.tts.speaking = true;
      State.tts.paused = false;
      const btn = document.getElementById('btnTts') || document.getElementById('ttsBtn');
      if(btn) btn.classList.add('active');
      
      for(let i=State.tts.idx; i<State.tts.queue.length; i++){
        if(!State.tts.speaking) break;
        State.tts.idx = i;
        highlightSentence(i);
        const progress = document.getElementById('progressFill');
        if(progress) progress.style.width = ((i/State.tts.queue.length)*100)+'%';
        const label = document.getElementById('pageLabel') || document.getElementById('progressText');
        if(label) label.textContent = `${i+1} / ${State.tts.queue.length}`;
        await speakOnce(State.tts.queue[i]);
      }
  ...
  window.stopTTS = window.stopTts = function(){
    State.tts.speaking=false;
    speechSynthesis.cancel();
    clearHighlight();
    const btn = document.getElementById('btnTts') || document.getElementById('ttsBtn');
  ...
  window.resumeTTS = function(){
    if(speechSynthesis.paused){
      speechSynthesis.resume();
      State.tts.speaking=true;
      // continue queue from current idx
      (async()=>{
        for(let i=State.tts.idx; i<State.tts.queue.length; i++){
          if(!State.tts.speaking) break;
          State.tts.idx=i;
  ...
  // Bind TTS button if exists
  document.addEventListener('DOMContentLoaded', ()=>{
    const btn = document.getElementById('btnTts');
    if(btn){
      btn.onclick = ()=>{
        if(State.tts.speaking) { window.stopTTS(); }
        else { window.startTts(); }
      };
    }
  });
})();
